// Kullanıcı stratejisini izole Web Worker'da çalıştırır.
// Worker'ın DOM/network erişimi yoktur → güvenli. Sunucuya hiçbir şey gitmez.
"use client";
import { Candle, Signal } from "./indicators";

const WORKER_SRC = `
// --- Sandbox indikatör kütüphanesi (kullanıcı erişebilir) ---
...
self.onmessage = function(e){
  ...
  try {
    var fn = new Function("candles","ema","sma","rsi","macd","atr","bollingerBands","makeSignal",
      code + "\\n;return (typeof strategy==='function')?strategy(candles):makeSignal({reason:'strategy() fonksiyonu bulunamadı'});");
    var result = fn(candles, ema, sma, rsi, macd, atr, bollingerBands, makeSignal);
    if(!result || typeof result!=="object" || !("signal" in result)){
      result = makeSignal({reason:"Geçersiz dönüş - bir Signal objesi döndürün"});
    }
...
export function runUserStrategy(code: string, candles: Candle[], timeoutMs = 3000): Promise<{ ok: boolean; signal?: Signal; error?: string }> {
  return new Promise((resolve) => {
    ...
    try {
      ...
      const timer = setTimeout(() => finish({ ok: false, error: "Zaman aşımı (sonsuz döngü?)" }), timeoutMs);
...
export const DEFAULT_USER_CODE = `// Kendi stratejini yaz! "strategy(candles)" fonksiyonu bir Signal döndürmeli.
// Kullanabileceğin hazır fonksiyonlar:
...
//
// candles: [{ time, open, high, low, close, volume }, ...]  (en yeni = son eleman)
...
function strategy(candles) {
  ...
  // EMA9, EMA21'i yukarı keserse LONG
  if (fast[i] > slow[i] && fast[i-1] <= slow[i-1]) {
    ...
    return makeSignal({
      ...
      confidence: 0.7,
      reason: "EMA9 EMA21'i yukarı kesti"
    });
  }
  ...
  if (fast[i] < slow[i] && fast[i-1] >= slow[i-1]) {
    ...
    return makeSignal({
      ...
      take_profit: [price - risk*1.5, price - risk*2.5, price - risk*4],
      confidence: 0.7,
      reason: "EMA9 EMA21'i aşağı kesti"
    });
  }

  return makeSignal({ reason: "Sinyal yok" });
}`;
